...
parfor ii = 1:num_channels
    ...
    for jj = 1:num_behaviors
        ...
        all_channels(ii).behavior_spectra(jj).before_start.time...
            = [start-.500, start - .001];
        all_channels(ii).behavior_spectra(jj).instruction_early_sec.time...
            = [instruction + .001, instruction + .500];
        all_channels(ii).behavior_spectra(jj).instruction_late_sec.time...
            = [instruction + .501, instruction + 1.000];
        all_channels(ii).behavior_spectra(jj).execution_sec.time...
